...
the camera intrinsics and the `T_pelvis_camera` extrinsic -- to a self-contained `.npz`,
so the deproject -> segment -> cloud (-> GraspGenX) pipeline can be re-run with no robot.
Replay with `10_segment.py --frame <file>.npz`. Also writes `_rgb.png` / `_depth.png`
previews next to the npz.
