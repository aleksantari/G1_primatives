#!/usr/bin/env python
"""tools/segment - SAM3 segmentation tool (NO motion). Get a head-cam frame (live), a
captured .npz, or a static image, refine a SAM3 mask (interactive cv2 GUI, or one auto
call), and report the masked single-object point cloud. For developing segmentation +
cloud offline.

  --frame PATH.npz    replay a captured frame (tools/capture_frame): rgb+depth+intrinsics+
  ...
  --save              write <base>_cloud.ply (downsampled, COLORED object cloud) + _mask.npy
                      + _overlay.png next to the frame -> feed tools/graspgen_viz --pcd

Live mode needs a head depth stream; all modes need a running SAM3 server
(python -m sam3.serving --port 5557).

  frame: bash -ic 'use_conda g1_curobo && python scripts/tools/segment.py --frame captures/scene1.npz --mode interactive'
  image: bash -ic 'use_conda g1_curobo && python scripts/tools/segment.py --image ~/repos/sam3/assets/images/groceries.jpg --mode auto --text "a bottle"'
  live : bash -ic 'use_conda g1_curobo && python scripts/tools/segment.py --target real --mode interactive'
"""
import argparse
import os
import time
...
def _load_frame(path):
    """Load a captured .npz (tools/capture_frame) -> (rgb, depth, intrinsics, T_pelvis_camera)."""
    z = np.load(path)
...
def main():
    ...
    ap.add_argument("--frame", default=None,
                    help="replay a captured .npz (tools/capture_frame): rgb+depth+cloud, no robot")
    ...
    ap.add_argument("--save", action="store_true",
                    help="save the mask + segmented cloud + overlay (feeds tools/graspgen_viz)")
    ...
    # --- source the frame: captured .npz (offline) | static image | live camera ---
    ...
    if args.frame:
        ...
        if not robot.camera.has_depth:
            print(f"[{args.target}] no head depth stream -- need the real ZED / sim depth PUB "
                  f"(or use --image).")
            return
        if not robot.wait_for_frames(rgb=True, depth=True):   # warm up both cold streams
            print("no color+depth after warmup -- is that stream publishing? "
                  "(depth = checks/03_depth, color = checks/02_camera; both must show frames)")
            return
        rgb, depth = robot.rgb(), robot.depth()
    ...
    # cloud only when we have depth (live or a captured --frame): the mask is applied
    ...
    # --- save artifacts for the offline GraspGenX demo (tools/graspgen_viz --pcd) ---
    if args.save:
        base = _out_base(args)
        ...
        if saved:
            ...
            if obj is not None and not obj.is_empty():
                print(f"  next: python scripts/tools/graspgen_viz.py --pcd {base}_cloud.ply")
        else:
