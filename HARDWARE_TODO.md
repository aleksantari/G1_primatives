...
> `--target real`), run **in order** — `checks/01_dds` (read-only) and `checks/02_camera` are
> safe/no-motion, `03`–`05`/`07`/`09` command the arms/hands, `06`/`08`/`10`/`11`/`12` are camera-only
> (`08` = head **depth** feed; `10` = SAM3 **segmentation**; `tools/capture_frame` saves an offline
> fixture; `tools/check_world` = depth-ESDF collision world inspector — no planning, no motion;
> `tools/graspgen_viz` needs no robot). `examples/02_pick` is the GraspGenX pick+lift; `09 --source
> graspgenx` + `10`/`tools/graspgen_viz` need the **GraspGenX** (`:5556`) / **SAM3** (`:5557`) ZMQ
> servers up (started in their own repos/envs — launch cmds in README "Grasp pipeline"). `scripts/tools/hand_loop_diag.py` is the low-level hand
...
- **Debug mode is operator-set via the physical remote**; since the api_dev restructure,
  `connect("real")` runs the CHECK-FIRST `hardware.ensure_debug_mode()` — an operator-set
  debug mode is detected and left UNTOUCHED (blind `ReleaseMode` dropped the robot out of
  low-level control on 06-18); an active ai/loco mode is released. `enter_debug_mode=False`
  skips the check. The A/B below still needs a hardware run.
...
| cuRobo world model | table / obstacles | **Depth-ESDF collision world wired + real-validated 2026-06-30** (head depth → cuRobo Mapper → ESDF, `motion/collision_world.py`, `planner.yaml: grasp.collision_world`); still **OFF by default** (`enabled: false`), opt in via `examples/02_pick --collision-world` — see "Depth-ESDF collision world" under Composite tasks. A static table/obstacle world is still not configured separately; the ESDF is the live-perception alternative. |

---

## Robot interface bring-up
- [ ] **DDS connectivity** — `g1_primitives.connect("real")` (robot.yaml DDS, mode=debug)
      constructs the arm + Dex3 controllers; reading `arm.get_current_dual_arm_q()`
      returns live joint q from the suspended G1. For a READ-ONLY check first (no controllers,
      no motion), run `scripts/checks/01_dds.py --target real`.
- [ ] **Launch home — WIRED (default).** `connect()` drives the arms to
...
- [ ] **Debug mode — CHECK-FIRST AUTO on connect (A/B untested on hardware).**
      `connect("real")` runs `hardware.ensure_debug_mode()`: CheckMode FIRST — an empty mode
      name (operator already set debug via the remote) returns "already in debug (untouched)"
      WITHOUT calling ReleaseMode (the 06-18 footgun: releasing an already-debug robot dropped
      `rt/lowcmd`, launch home residual ~87° = zero motion); a named ai/loco mode is released in
      a bounded loop. `enter_debug_mode=False` skips the check entirely.
      **Hardware A/B to run:** (A) operator-set debug → connect must log "already in debug
      (untouched)" and the arms respond; (B) robot left in AI/loco mode → connect must release
      into debug and the arms respond. This is the FIRST real-hardware test of the api_dev branch.
- [ ] **Hand state** — `python scripts/tools/hand_loop_diag.py --side left|right` opens/closes each hand and
...
- [x] **Speed — `time_dilation` knob in `planner.yaml` (2026-06-17).** cuRobo plans ~1.1 rad/s; the
      ...
      path/goal, lower velocity + quadratically lower accel. Per-run: `checks/05_move --speed`; dial toward
...
- [x] **Gravity comp — VALIDATED ON HARDWARE, default ON for real (2026-06-17).** cuRobo RNEA
      ...
      mechanism in **`docs/gravity_comp.md`**). Ramp/re-check via `checks/05_move --gravity-scale`.
- [ ] **Trajectory speed vs tracking — WATCH on real grasps; root fix DEFERRED** (diagnosed in sim
      ...
      **approach** (curves around the obstacle) is dynamic enough that the executor's tracking-error
      ...
      `examples/02_pick --speed 0.5` clears it. **Real is the HARDER target, not safe-by-default:** it runs
...
## Hand grasp tuning
- [ ] **Presets** — command `open`/`power_close`/`pinch` with `tools/hand_loop_diag.py`, read back q, fix the
      ...
      **Tuned 2026-06-30:** `examples/02_pick --close-frac` default raised 0.5 → 0.65 (firmer close on the
...
- [x] **Image client — WIRED.** Real head = ZED stereo: `connect("real")` picks
      `camera_real.yaml` opens the vendored ZMQ client, slices the 720×2560 side-by-side frame to a
...
- [x] **Head DEPTH — WIRED (2026-06-23).** A dedicated raw-float32 ZMQ stream (server port 56555,
      ...
      **Feed check:** `python scripts/checks/03_depth.py --target real` (mm stats + colorized view).
...
- [ ] **Feed check** — `python scripts/checks/02_camera.py --target real` shows one sliced ZED eye.
...
- [x] **Grasp pipeline (GraspGenX + SAM3) — BUILT, offline-tested** (`grasp/`,
      ...
      `scripts/examples/02_pick.py` + `scripts/tools/segment.py`): `GraspSource` seam (`grasp.yaml:
      source` = `graspgenx` | `sim_cloud`; the apriltag source is deleted); GraspGenX path = head depth → masked
      ...
      (`grasp_motion` → `plan_grasp_set_sweep`: goalset pick + approach/grasp/lift). Standalone ZMQ clients (no service import). **SAM3
      segmentation validated on static images** (`tools/segment --image`); the offline demo
      (`tools/segment --save` → COLORED `.ply` → `tools/graspgen_viz`) renders cloud + grasps in viser.
  - [x] **Sim de-risk (`--source sim_cloud`) — DONE 2026-06-28**: in Isaac, a GT cube cloud from
        ...
        NO ZED/SAM3 (only the GraspGenX server + the sim). `examples/02_pick --target sim --source
        ...
        Needs `perception.yaml: source: sim_state`.
  - [x] **Real grasp run — DONE, real-validated 2026-06-30.** The full GraspGenX pick+lift ran on the
        physical G1: live ZED depth → SAM3 segmentation (`tools/segment` / `09 --segment interactive`) →
        GraspGenX (`source: graspgenx`, `planner: topdown`) → the kinematics-DERIVED grasp→wrist
        tool transform → cuRobo native `plan_grasp` → approach → grasp → close → lift. The robot
        picked + lifted the object. (Run as `examples/02_pick --target real --source graspgenx --segment
        interactive`, with `tools/segment --target real` first to confirm the live mask + masked point
  ...
  - [x] **Depth-ESDF collision world — DONE, real-validated 2026-06-30** (commits
        ...
        Exercised WITH `examples/02_pick --collision-world` IN the real grasp (no longer sim-only): the
        ...
        and `tools/check_world --target real` validated the geometry placement + self-filter on the real
        ZED depth. Head
        ...
        flag or `examples/02_pick --collision-world`. Inspect IN ISOLATION (no planning/motion) with
        `scripts/tools/check_world.py --target real` (`--visualize` viser overlay; `--probe X Y Z` is the
