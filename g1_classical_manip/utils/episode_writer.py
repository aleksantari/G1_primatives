...
class EpisodeWriter:
    def __init__(
        ...
        self.is_available = (
            True  # Indicates whether the class is available for new operations
        )
    ...
    def data_info(self, version="1.0.0", date=None, author=None):
        self.info = {
            ...
            "date": (
                datetime.date.today().strftime("%Y-%m-%d") if date is None else date
            ),
    ...
    def create_episode(self):
        ...
        with open(self.json_path, "w", encoding="utf-8") as f:
            f.write("{\n")
            f.write(
                '"info": ' + json.dumps(self.info, ensure_ascii=False, indent=4) + ",\n"
            )
            f.write(
                '"text": ' + json.dumps(self.text, ensure_ascii=False, indent=4) + ",\n"
            )
            f.write('"data": [\n')
        ...
        while not self.stop_worker or not self.item_data_queue.empty():
            ...
            try:
                ...
                try:
                    ...
                    logger_mp.info(
                        f"Error processing item_data (idx={item_data['idx']}): {e}"
                    )
    ...
    def _process_item_data(self, item_data):
        ...
        if colors:
            for idx_color, (color_key, color) in enumerate(colors.items()):
                ...
                if color is None or getattr(color, "size", 0) == 0:
                    logger_mp.info(f"Skip empty color image: idx={idx} key={color_key}")
                    continue
        ...
        if depths:
            for idx_depth, (depth_key, depth) in enumerate(depths.items()):
                ...
                if depth is None or getattr(depth, "size", 0) == 0:
                    logger_mp.info(f"Skip empty depth image: idx={idx} key={depth_key}")
                    continue
                if getattr(depth, "dtype", None) != np.uint16:
                    logger_mp.info(
                        "Skip non-uint16 depth image: "
                        f"idx={idx} key={depth_key} dtype={getattr(depth, 'dtype', None)}"
                    )
                    continue
        ...
        if self.rerun_log:
            ...
            logger_mp.info(
                f"==> episode_id:{self.episode_id}  item_id:{idx}  current_time:{curent_record_time}"
            )
    ...
    def close(self):
        ...
        ):  # If self.is_available is False, it means there is still data not saved.
            self.save_episode()
